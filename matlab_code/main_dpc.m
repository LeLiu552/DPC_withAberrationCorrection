function [amplitude, phase, pupil, pupilphase, source, fIDPC] = main_dpc_DM(IDPC,aberration_correction)
...
%     aberration_correction = true;
    ...
    %     load('D:\temp_data\IDPCdata.mat')
    % else
    %     load('..\sample_data\dataset_DPC_MCF10A.mat');
    % %     load('D:\temp_data\TestDora\TestSnap_DPC_1\testData.mat');
    ...
    dim           = [size(IDPC, 1), size(IDPC, 2)]; % image size
    sigma         = 1.05;                            % partial coherence factor
    na            = 0.95;                           % numerical aperture of the imaging system
    na_illum      = sigma*na;                       % numerical aperture of the illumination
    magnification = 40;                           % magnification of the imaging system
    lambda        = 0.625;                          % wavelength in micron
    ps            = 6.45/magnification;              % pixel size in micron
    wavenumber    = 2*pi/lambda;                    % wave number


    if aberration_correction
        illu_rotation = [0, 180, 90];               % orientation of the illumination
%         illu_rotation = [180, 0, 270];
    else
        illu_rotation = [0, 180, 90, 270];
    %     illu_rotation = [270, 180, 90, 0];
    end
